...
def insert_job(job_id, company_id, title, description, location, salary, job_type, posted_date):
    ...
    data = (job_id, company_id, title, description, location, salary, job_type, posted_date)
    conn = get_connection()
    cursor = conn.cursor()
...
def insert_applicant(applicant_id, first_name, last_name, email, phone, resume):
    data = (applicant_id, first_name, last_name, email, phone, resume)
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO Applicants VALUES (?, ?, ?, ?, ?, ?)", data)
    conn.commit()
    conn.close()

def insert_application(app_id, job_id, applicant_id, application_date, cover_letter):
    data = (app_id, job_id, applicant_id, application_date, cover_letter)
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO Applications VALUES (?, ?, ?, ?, ?)", data)
    conn.commit()
    conn.close()
